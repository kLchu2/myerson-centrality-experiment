...
#### ウォークベース修正版Myerson中心性

**定義**: $Y^{mod}_G(v) = \displaystyle \sum_{l=1}^{L} \frac{s_i(l)}{l+1} \cdot r^l$

- $s_i(l)$: 頂点 $i$ を含む長さ $l$ の**全てのウォーク**の出現回数（中間点含む）
- $L$: 計算対象の最大ウォーク長
- $r$: 影響定数パラメータ
...
│   ├── myerson.py           # 通常版Myerson中心性（最短パスベース）
│   ├── revised_myerson.py   # 改訂版Myerson中心性（単純パスベース）
│   ├── path_limited_myerson.py # パス長制限Myerson中心性
│   └── modified_myerson.py  # ウォークベース修正版Myerson中心性
...
│   ├── time_comparison.py            # 計算時間比較実験
│   └── compare_centrality.py         # アルゴリズム間の比較・検証
├── tests/                   # テストコード
│   ├── **init**.py
│   ├── test_myerson.py
...
### 計算時間比較（time_comparison.py）

各手法（マイヤーソン、改訂版、パス長制限）の計算時間をグラフサイズを変えながら比較します。

```bash
python experiments/time_comparison.py
```

- 結果は `results/json/` に保存され、コンソールに要約が表示されます。

### 手法間の比較（compare_centrality.py）

異なる中心性指標間の計算結果を比較し、相関などを確認します。

```bash
python experiments/compare_centrality.py
```
